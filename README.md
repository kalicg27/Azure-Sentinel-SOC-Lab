...
![ArchitectureVNET](architecture/vnet-subnets.png)
![ArchitectureVM](architecture/VM-list.png)
...
![Results From Quory KQL](detections/Overal_result_detection.png)

---

## 💥 Threat Profiles, Simulations & MITRE ATT&CK Mapping

With logging validated, offensive operations were generated from the Kali Linux platform to test defensive visibility against specific MITRE ATT&CK patterns.

### 1. MITRE T1110 - Brute Force (Credential Access)

- **Adversary Action:** Utilizing **Hydra** over the RDP protocol, the attacker node executed a targeted password-spraying campaign using automated user matrices (`users.txt`, `passwords.txt`) to simulate unauthorized access attempts.
- **Telemetry Evidence:** Captured in mass by Windows Security Event ID **4625 (An account failed to log on)**.

### 2. MITRE T1059 / T1087 - Command Execution & Account Discovery (Reconnaissance)

- **Adversary Action:** Simulating a post-compromise environment, reconnaissance commands were triggered directly inside the server's command-line interface:
